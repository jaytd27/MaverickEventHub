...
public class Exhibition extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Button button_help = (Button) findViewById(R.id.btn_exhibition_help);
		Button button_close = (Button) findViewById(R.id.btn_exhibition_close);
		Button button_create = (Button) findViewById(R.id.btn_exhibition_create);
		TextView textview_exhibition = (TextView) findViewById(R.id.tv_exhibition);
		...
		TextView textview_time = (TextView) findViewById(R.id.tv_exhibition_time);
		...
		TextView textview_details = (TextView) findViewById(R.id.tv_exhibition_details);
		TextView textview_extras = (TextView) findViewById(R.id.tv_exhibition_extras);
		...
		EditText edittext_details = (EditText) findViewById(R.id.et_exhibition_details);
		EditText edittext_extras = (EditText) findViewById(R.id.et_exhibition_extras);
		DatePicker dp_date = (DatePicker) findViewById(R.id.dp_exhibition_date);
		//TimePicker tp_time = (TimePicker) findViewById(R.id.tp_exhibition_time);
		
		//Set the min date for date picker to be after today
		...
		button_help.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
            	Exhibition.this.runOnUiThread(new Runnable() {
    	            public void run() {
    	                AlertDialog.Builder builder = new AlertDialog.Builder(Exhibition.this,R.style.AlertDialogTheme);
	        		...
	        		if(type.equals("")&&count==1)
	        		{
	        			Exhibition.this.runOnUiThread(new Runnable() {
	        	            public void run() {
	        	                ...
	        	                builder.setMessage("Please enter the type of exhibition")  
	        	                       .setCancelable(false)
	        	                       .setPositiveButton("OK", new DialogInterface.OnClickListener() {
	        	                           public void onClick(DialogInterface dialog, int id) {
	        	                           }
	                 ...
	                 new Thread(new Runnable() {
	                        public void run() {    
	                        	runOnUiThread(new Runnable() {
	                                public void run() {
	                                	Toast.makeText(Exhibition.this,"Creating Event...", Toast.LENGTH_SHORT).show();
	                                }
	                            });

	                            createexhibitionEvent();                          
	                        }
	                      }).start();               
	        		}   
		     }
	...
	void createexhibitionEvent(){
        try{            
        	runOnUiThread(new Runnable() {
                public void run() {
                	//Toast.makeText(Exhibition.this,"Inside...", Toast.LENGTH_SHORT).show();
                }
            });
        	EditText edittext_name = (EditText) findViewById(R.id.et_exhibition_name);
	 		EditText edittext_type = (EditText) findViewById(R.id.et_exhibition_type);
	        ...
	        final String exhibition_timestamp=sb.toString();
	        
	        runOnUiThread(new Runnable() {
                public void run() {
                	//Toast.makeText(Exhibition.this,exhibition_timestamp, Toast.LENGTH_SHORT).show();
                }
            });
            ...
            httppost= new HttpPost("http://omega.uta.edu/~jxm6478/createexhibition.php"); // make sure the url is correct.
            //add your data
            ...
            nameValuePairs.add(new BasicNameValuePair("date",exhibition_timestamp));
            ...
            if(response.toString().equalsIgnoreCase("Error in Query")){
            	...
            	runOnUiThread(new Runnable() {
                    public void run() {
                        //Toast.makeText(Exhibition.this,"Event Created", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                });
